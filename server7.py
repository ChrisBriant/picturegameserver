...
class BroadcastServerFactory(WebSocketServerFactory):
    ...
    #Helper function to remove room data from client
    def remove_room_assignment(self,client_id):
    ...
    def calc_knockout(self,round,scores,game,room,room_name):
        #Control variable to enable game deletion
        winbyknockout = False
        if round > 1:
            ...
            if len(knockouts) > 0:
                for player in knockouts:
                    ...
                    self.remove_room_assignment(player['player'])
    ...
    def play_card(self,room_name,card,client_id):
        ...
        if len(game['trick']) + 1 == len(room['members']):
            ...
            if game['trick_count'] == 0:
                ...
                winbyknockout = self.calc_knockout(game['round_number'] - 1,round_result['scores'],game,room,room_name)
                deck = random.sample(CARD_SET,len(CARD_SET))#
                hands, trump = deal(deck,deal_amount,room['members'])
                game['trump'] = trump
                game['hands'] = hands
