...
from autobahn.asyncio.websocket import WebSocketServerFactory, WebSocketServerProtocol #, listenWS
from autobahn.twist.resource import WebSocketResource
...
class BroadcastServerProtocol(WebSocketServerProtocol):
    async def onOpen(self):
        await self.factory.register(self)
...
class BroadcastServerFactory(WebSocketServerFactory):
    ...
    async def store_object(self,id,obj):
        obj_to_store = json.dumps(obj)
        await R.set(id,obj_to_store)

    async def get_from_store(self,id):
        obj = R.get(id)
        print('This is from redis',obj)
        dict_obj = ast.literal_eval(obj)
        return obj

    async def get_clients_from_store(self):
        cli_list = []
    ...
    async def register(self, client):
        ...
        if client not in registered:
            ...
            await self.store_object(cid,client_obj)
            payload = {
                'type': 'register',
                'yourid':cid
            }
    ...
    async def send_client_list(self):
        ...
        clients_data = await self.get_clients_from_store()
        payload = {
            'type' : 'client_list',
    ...
    async def set_name(self,client_id,name):
        client_obj = await self.get_from_store(client_id)
